...
export default async function ReportPreviewPage({ params }: PageProps) {
  try {
    ...
    const pdcaItemsList = Array.isArray(report.pdcaItems) ? report.pdcaItems : []
    const pdcaByPhase = {
      P: pdcaItemsList.filter((i: any) => i?.phase === 'P'),
      D: pdcaItemsList.filter((i: any) => i?.phase === 'D'),
      C: pdcaItemsList.filter((i: any) => i?.phase === 'C'),
      A: pdcaItemsList.filter((i: any) => i?.phase === 'A'),
    }

    const signatoriesList = Array.isArray(report.signatories) ? report.signatories : []
    const sigReporter = signatoriesList.find((s: any) => s?.role === 'REPORTER')
    const sigPlanHead = signatoriesList.find((s: any) => s?.role === 'PLAN_HEAD')
    const sigPrincipal = signatoriesList.find((s: any) => s?.role === 'PRINCIPAL')

    const totalSatisfactionAvg = report.satisfactionSurvey?.avgTotal || 0

    // Evidence Documents Map
    const evidenceDocsList = Array.isArray(report.evidenceDocs) ? report.evidenceDocs : []
    const evidenceMap: Record<string, boolean> = {}
    for (const doc of evidenceDocsList) {
      if (doc?.docType) evidenceMap[doc.docType] = Boolean(doc.hasDoc)
    }

    const activityPhotosList = Array.isArray(report.activityPhotos) ? report.activityPhotos : []
              ...
              <div className="p-10 border-b border-slate-100">
                ...
                <div className="grid grid-cols-3 gap-4 text-center mb-4">
                  <div className="border rounded-xl p-4 bg-blue-50">
                    ...
                    <p className="text-xl font-bold text-blue-800">{formatCurrency(report.budget.approved || 0)}</p>
                  </div>
                  <div className="border rounded-xl p-4 bg-orange-50">
                    <p className="text-xs text-orange-500 mb-1">งบที่ใช้ไป</p>
                    <p className="text-xl font-bold text-orange-800">{formatCurrency(report.budget.used || 0)}</p>
                  </div>
                  <div className="border rounded-xl p-4 bg-green-50">
                    <p className="text-xs text-green-500 mb-1">งบคงเหลือ</p>
                    <p className="text-xl font-bold text-green-800">{formatCurrency(report.budget.remaining || 0)}</p>
                  </div>
                </div>
                ...
                <table className="w-full text-sm border border-slate-200 rounded-lg overflow-hidden">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {['P', 'D', 'C', 'A'].map(phase => {
                      ...
                      const phaseColors: Record<string, string> = { P: 'bg-blue-50', D: 'bg-green-50', C: 'bg-yellow-50', A: 'bg-purple-50' }
                      const phaseLabels: Record<string, string> = { P: 'P: วางแผน', D: 'D: ดำเนิน', C: 'C: ประเมิน', A: 'A: สรุปผล' }
                      return phaseItems.map((item: any, i: number) => (
                        <tr key={item?.id || i} className={phaseColors[phase]}>
            ...
            {Array.isArray(report.objectives) && report.objectives.length > 0 && (
              <div className="p-10 border-b border-slate-100">
                ...
                <table className="w-full text-sm border border-slate-200">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {report.objectives.map((obj: any, idx: number) => (
                      <tr key={obj?.id || idx} className="hover:bg-slate-50">
              ...
              {activityPhotosList.length > 0 && (
                <div className="mt-6 pt-6 border-t border-slate-100">
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    {activityPhotosList.map((photo: any, index: number) => {
