...
async function safeQuery<T>(fn: () => Promise<T>, fallback: T): Promise<T> {
  try {
    ...
    console.error('D1 Query warning (using fallback):', e)
    return fallback
  }
}
...
export async function getReportById(id: string) {
  ...
  const [project, budget, pdcaItems, objectives, achievementScore,
    satisfactionSurvey, reportSummary, evidenceDocs, activityPhotos, signatories, comments] =
    await Promise.all([
      safeQuery(() => d1.prepare(`SELECT * FROM "Project" WHERE reportId = ?`).bind(id).first(), null),
      safeQuery(() => d1.prepare(`SELECT * FROM "Budget" WHERE reportId = ?`).bind(id).first(), null),
      safeQuery(() => d1.prepare(`SELECT * FROM "PdcaItem" WHERE reportId = ? ORDER BY "order" ASC`).bind(id).all(), { results: [] }),
      safeQuery(() => d1.prepare(`SELECT * FROM "Objective" WHERE reportId = ? ORDER BY "order" ASC`).bind(id).all(), { results: [] }),
      safeQuery(() => d1.prepare(`SELECT * FROM "AchievementScore" WHERE reportId = ?`).bind(id).first(), null),
      safeQuery(() => d1.prepare(`SELECT * FROM "SatisfactionSurvey" WHERE reportId = ?`).bind(id).first(), null),
      safeQuery(() => d1.prepare(`SELECT * FROM "ReportSummary" WHERE reportId = ?`).bind(id).first(), null),
      safeQuery(() => d1.prepare(`SELECT * FROM "EvidenceDocument" WHERE reportId = ?`).bind(id).all(), { results: [] }),
      safeQuery(() => d1.prepare(`SELECT * FROM "ActivityPhoto" WHERE reportId = ? ORDER BY "order" ASC`).bind(id).all(), { results: [] }),
      safeQuery(() => d1.prepare(`SELECT * FROM "Signatory" WHERE reportId = ?`).bind(id).all(), { results: [] }),
      safeQuery(() => d1.prepare(`
  ...
  if (satisfactionSurvey) {
    const answers = await safeQuery(() => d1.prepare(`SELECT * FROM "SatisfactionAnswer" WHERE surveyId = ?`)
      .bind((satisfactionSurvey as any).id).all(), { results: [] })
...
export async function updateReport(id: string, body: any) {
  ...
  if (reportUpdates.length > 0) {
    await safeQuery(() => d1.prepare(`UPDATE "Report" SET ${reportUpdates.join(',')} WHERE id=?`)
      .bind(...reportVals, id).run(), null)
  }
  ...
  if (body.activityPhotos) {
    await safeQuery(async () => {
      ...
      for (const photo of body.activityPhotos || []) {
        ...
        let photoUrl = photo.url.trim()
        if (photoUrl.length > 70000) photoUrl = photoUrl.slice(0, 70000)
        await d1.prepare(`INSERT INTO "ActivityPhoto" (id,reportId,url,caption,"order",includeInReport,layout,createdAt) VALUES (?,?,?,?,?,?,?,?)`)
          .bind(newId(), id, photoUrl, photo.caption || '', order++, photo.includeInReport !== false ? 1 : 0, photo.layout || 'TWO_PER_PAGE', now).run()
      }
    }, null)
  }
...
export async function deleteReport(id: string) {
  const d1 = getD1()
  if (!d1) throw new Error('D1 not available')
  await safeQuery(() => d1.prepare(`DELETE FROM "Report" WHERE id=?`).bind(id).run(), null)
}
...
export async function updateReportStatus(id: string, status: string, comment?: string, authorId?: string) {
  ...
  const now = new Date().toISOString()
  await safeQuery(() => d1.prepare(`UPDATE "Report" SET status=?, updatedAt=? WHERE id=?`).bind(status, now, id).run(), null)

  if (comment && authorId) {
    await safeQuery(() => d1.prepare(`INSERT INTO "WorkflowComment" (id, reportId, content, authorId, createdAt) VALUES (?,?,?,?,?)`)
